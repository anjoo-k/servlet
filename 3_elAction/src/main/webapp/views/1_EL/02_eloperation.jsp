...
<body>
	...
	<h3>3. 동등 비교</h3>
	<!-- el에서 == 비교는 자바에서 equals()와 같은 동작 -->
	sOne과 sTwo가 일치합니까? ${sOne == sTwo } 또는  ${sOne eq sTwo } <br>
	sOne과 sTwo가 일치하지 않습니까? ${sOne != sTwo } 또는  ${sOne ne sTwo } <br>	

	big에 담긴 값이 10과 일치합니까? ${big == 10 } 또는 ${big eq 10 } <br><br>
	
	
	sThree에 담긴 값이 '안녕'과 일치합니까? ${sThree == "안녕" } <br>
	sThree에 담긴 값이 'HI'과 일치합니까? ${sThree == 'HI' } <br><br>
	<!-- el 구문 안에서 문자열 리터럴값은 큰따옴표 작은따옴표를 가리지 않음 -->
	
	<h3>4. 객체가 null인지 아닌지 리스트가 비었는지 비교</h3>
